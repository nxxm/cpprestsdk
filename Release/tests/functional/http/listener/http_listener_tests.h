...
{
public:
    uri_address() : m_uri(_XPLATSTR("http://localhost:34567/")), m_secure_uri(_XPLATSTR("https://localhost:8443/"))
    {
        if (!s_dummy_listener)
            s_dummy_listener =
                std::make_shared<web::http::experimental::listener::http_listener>(_XPLATSTR("http://localhost:30000/"));
    }
