...
SUITE(status_code_reason_phrase_tests)
{
    TEST_FIXTURE(uri_address, status_codes)
    {
        ...
        VERIFY_ARE_EQUAL(0u, p_client->request(methods::PUT, _XPLATSTR("")));
        p_client->next_response()
    ...
    TEST_FIXTURE(uri_address, reason_phrase)
    {
        ...
        p_client->next_response()
            .then([](test_response* p_response) {
                ...
                VERIFY_ARE_EQUAL(_XPLATSTR("Not Modified"), p_response->m_reason_phrase);
            })
            .wait();
        ...
        listener.support([](http_request request) {
            ...
            response.set_reason_phrase(_XPLATSTR("Custom"));
        ...
        p_client->next_response()
            .then([](test_response* p_response) {
                http_asserts::assert_test_response_equals(p_response, status_codes::NotModified);
                VERIFY_ARE_EQUAL(_XPLATSTR("Custom"), p_response->m_reason_phrase);
            })
            .wait();
        ...
        p_client->next_response()
            .then([](test_response* p_response) {
                ...
                VERIFY_ARE_EQUAL(_XPLATSTR(""), p_response->m_reason_phrase);
            })
            .wait();
