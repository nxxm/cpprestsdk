...
SUITE(building_request_tests)
{
    TEST_FIXTURE(uri_address, simple_values)
    {
        ...
        // Set a path once.
        const utility::string_t custom_path1 = _XPLATSTR("/hey/custom/path");
        ...
        msg.set_request_uri(custom_path1);
        VERIFY_ARE_EQUAL(custom_path1, msg.relative_uri().to_string());
        const utility::string_t custom_path2 = _XPLATSTR("/yes/you/there");
    ...
    TEST_FIXTURE(uri_address, body_types)
    {
        ...
        // Body data types.
        const method method(_XPLATSTR("CUSTOMmethod"));
        utility::string_t str_body(_XPLATSTR("YES_BASIC_STRING BODY"));
        ...
        utility::string_t custom_content = _XPLATSTR("YESNOW!");

        // vector - no content type.
        http_request msg(method);
        msg.set_body(std::move(vector_move_body));
        VERIFY_ARE_EQUAL(_XPLATSTR("application/octet-stream"), msg.headers()[_XPLATSTR("Content-Type")]);
        ...
        msg.headers().add(_XPLATSTR("Content-Type"), custom_content);
        msg.set_body(std::move(vector_move_body));
        VERIFY_ARE_EQUAL(custom_content, msg.headers()[_XPLATSTR("Content-Type")]);
        p_server->next_request().then([&](test_request* p_request) {
            auto received = p_request->m_body;
            auto sent = vector_body;
            VERIFY_IS_TRUE(received == sent);
        ...
        VERIFY_ARE_EQUAL(_XPLATSTR("text/plain; charset=utf-8"), msg.headers()[_XPLATSTR("Content-Type")]);
        p_server->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(
                p_request, method, _XPLATSTR("/"), _XPLATSTR("text/plain; charset=utf-8"), str_body);
            p_request->reply(200);
        });
        http_asserts::assert_response_equals(client.request(msg).get(), status_codes::OK);
        ...
        VERIFY_ARE_EQUAL(custom_content, msg.headers()[_XPLATSTR("Content-Type")]);
        p_server->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(p_request, method, _XPLATSTR("/"), custom_content, str_body);
            p_request->reply(200);
        });
        http_asserts::assert_response_equals(client.request(msg).get(), status_codes::OK);
    }
    ...
    TEST_FIXTURE(uri_address, empty_bodies)
    {
        ...
        p_server->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(p_request, method, _XPLATSTR("/"), _XPLATSTR("application/octet-stream"));
        ...
        p_server->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(p_request, method, _XPLATSTR("/"), _XPLATSTR("text/plain; charset=utf-8"));
            VERIFY_ARE_EQUAL(0u, p_request->m_body.size());
            p_request->reply(200);
        });
        http_asserts::assert_response_equals(client.request(msg).get(), status_codes::OK);
    ...
    TEST_FIXTURE(uri_address, set_body)
    {
        ...
        utility::string_t data(_XPLATSTR("YOU KNOW~!!!!!"));
        utility::string_t content_type = _XPLATSTR("text/plain; charset=utf-8");

        // without content type
        http_request msg(mtd);
        msg.set_body(data);
        VERIFY_ARE_EQUAL(content_type, msg.headers()[_XPLATSTR("Content-Type")]);
        ...
        // with content type
        content_type = _XPLATSTR("YESYES");
#ifdef _UTF16_STRINGS
        const utility::string_t expected_content_type = _XPLATSTR("YESYES; charset=utf-8");
#else
        const utility::string_t expected_content_type = _XPLATSTR("YESYES");
#endif
        msg = http_request(mtd);
        msg.set_body(data, content_type);
        VERIFY_ARE_EQUAL(expected_content_type, msg.headers()[_XPLATSTR("Content-Type")]);
        scoped.server()->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(p_request, mtd, _XPLATSTR("/"), expected_content_type, data);
            p_request->reply(200);
        });
        http_asserts::assert_response_equals(client.request(msg).get(), status_codes::OK);
    }
    ...
    TEST_FIXTURE(uri_address, set_content_length_locale, "Ignore:Android", "Locale unsupported on Android")
    {
        ...
        VERIFY_ARE_EQUAL(_XPLATSTR("1000"), req.headers()[web::http::header_names::content_length]); // fr_RF would have 1 000
    }
    ...
    TEST_FIXTURE(uri_address, set_port_locale, "Ignore:Android", "Locale unsupported on Android")
    {
        ...
        utility::string_t data(_XPLATSTR("STRING data 1000"));
        t = scoped.server()->next_request().then([&](test_request* p_request) {
            http_asserts::assert_test_request_equals(
                p_request, methods::PUT, _XPLATSTR("/"), _XPLATSTR("text/plain; charset=utf-8"), data);
            p_request->reply(200);
        });
    ...
    TEST_FIXTURE(uri_address, reuse_request)
    {
        ...
        {
            p_server->next_request().then([](test_request* p_request) {
                http_asserts::assert_test_request_equals(p_request, methods::GET, _XPLATSTR("/"));
                p_request->reply(200);
            });
            http_asserts::assert_response_equals(client.request(msg).get(), status_codes::OK);
        }
